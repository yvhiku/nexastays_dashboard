...
type Filter = "all" | "guest" | "host" | "suspended" | "banned";
...
export default function UsersPage() {
  ...
  const counts = useMemo(
    () => ({
      ...
      host: users.filter((u) => u.role !== "guest").length,
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Guest accounts from Identity, merged with Stays host profiles."
      />

      {error && (
        <p className="mb-4 text-sm text-nexa-danger">Failed to load users: {error}</p>
      )}
      {loading && (
        <p className="mb-4 text-sm text-nexa-ink-4">Loading users…</p>
      )}
      ...
      <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <FilterTabs<Filter>
          ...
          options={[
            ...
            { value: "host", label: "Hosts", count: counts.host },
