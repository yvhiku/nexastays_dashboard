...
test("dev CSP allows localhost aliases for loopback APIs", () => {
  const csp = buildDashboardCsp({
    isDev: true,
    identityApiUrl: "http://127.0.0.1:3001/api/v1",
    staysApiUrl: "http://127.0.0.1:3002/api/v1",
  ...
  assert.match(csp, /connect-src[^;]*http:\/\/127\.0\.0\.1:3001/);
  assert.match(csp, /connect-src[^;]*http:\/\/localhost:3001/);
  assert.match(csp, /connect-src[^;]*http:\/\/localhost:3002/);
});
