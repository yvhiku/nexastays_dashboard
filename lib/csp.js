...
/** localhost and 127.0.0.1 are different CSP origins; allow both for loopback APIs. */
function expandLoopbackOrigins(origin) {
  if (!origin) return [];
  try {
    const parsed = new URL(origin);
    if (!isLoopbackHostname(parsed.hostname)) return [origin];
    const aliases = ["localhost", "127.0.0.1"];
    return aliases.map((host) => {
      const copy = new URL(origin);
      copy.hostname = host;
      return copy.origin;
    });
  } catch {
...
function buildDashboardCsp(opts = {}) {
  ...
  for (const origin of [
    ...expandLoopbackOrigins(identityOrigin),
    ...expandLoopbackOrigins(staysOrigin),
  ]) {
  ...
  // next/font/local self-hosts under /_next/static — 'self' only.
  const fontSrc = ["'self'"];
